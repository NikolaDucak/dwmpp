...
void Client::resize(point xy, point wh) {
    // save pravious position and size
    ...
    // sent x new window configuration with mask which 
    // changes to applly
    m_xWindow.configureWindow(wc,
                              CWX | CWY | CWWidth | CWHeight | CWBorderWidth);

    // sent x configuration event, received by the WM later
...
void Client::setFullscreen(bool f) {
    ...
    if (m_fullscreen) {
        // save old border and and remove current border
        // since fullscreen clients don't have a border
...
void Client::setState(int state) {
    ...
    m_xWindow.changeProperty(xlib::WMState, xlib::WMState, (unsigned char*)data,
                             2);
}
